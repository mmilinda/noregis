...
function OcrProcessing({ image, mode, onDone, t }) {
  ...
  const [status, setStatus]     = useState(t.google_analysis || 'Analyse en cours…');
  ...
  useEffect(() => {
    ...
    const runBackendOCR = async () => {
      try {
        setStatus(t.google_analysis || 'Envoi de l\'image…');
        setProgress(20);
        ...
        if (!response.ok) {
          ...
          console.error('Détails erreur scan :', errorBody);
        ...
        if (!result.infosExtraites) throw new Error(t.no_text || 'Aucun texte extrait.');

        setStatus(t.extracting || 'Extraction terminée');
        setProgress(100);

        // On filtre les champs non-objet pour l'affichage (exclut "confiance")
        const champsAfficheables = {};
        for (const [key, value] of Object.entries(result.infosExtraites)) {
        ...
        if (isMounted) onDone({ error: t.cloud_fail || 'Échec de l\'analyse. Réessayez.' });
      }
    };
    ...
    <div className="flex flex-col items-center justify-center h-full gap-8 p-8 text-center bg-slate-50/50 dark:bg-slate-900/50">
      <div className="relative">
        <img
          src={image}
          alt="doc"
          className="w-52 h-32 object-cover rounded-xl border-2 border-brand-blue-bright/30 blur-[1px] opacity-40 shadow-2xl"
        />
      ...
      <div className="space-y-3">
        ...
        <div className="w-48 h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden mx-auto">
          <div
            className="h-full bg-brand-blue-bright transition-all duration-500"
            style={{ width: `${progress}%` }}
          />
        </div>
        <p className="text-[10px] font-black text-brand-blue-bright uppercase animate-pulse tracking-widest">
          {status}
        </p>
      </div>
    </div>
...
/* ===========================================
   SCANNER CAMÉRA LIVE (avec tentative flash)
   =========================================== */
          ...
          if (track) {
            ...
            if (caps?.torch === true) {
              await track
                ...
                .catch((e) => console.warn('Flash non activable :', e));
            }
          }
        }
      } catch (err) {
        if (facing === 'environment') {
          startCamera('user'); // fallback caméra frontale
        } else {
          setError(
            `${t.camera_error || 'Erreur caméra'} : ${
              err.name === 'NotAllowedError'
                ? t.permission_denied || 'permission refusée'
...
/* ===========================================
   SCAN PANEL — Composant principal
   =========================================== */
      ...
      <div className="p-4 border-b border-slate-50 dark:border-slate-800 flex items-center gap-4 bg-slate-50/50 dark:bg-slate-900/50">
        ...
        <div>
          ...
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-tighter mt-1.5">
            {mode === 'vehicule' ? t.license_plate || 'Plaque' : t.id_card || 'Pièce d\'identité'}
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-hidden overflow-y-auto">
        ...
        {phase === 'choose' && (
          <div className="p-8 flex flex-col gap-4 animate-in fade-in zoom-in-95 duration-300">
            <p className="text-center text-slate-500 text-[11px] font-extrabold uppercase tracking-widest mb-2">
              {t.scan_method || 'Choisir une méthode'}
            </p>
            ...
            >
              ...
              <div className="flex-1">
                <p className="font-black text-sm text-slate-900 dark:text-white">
                  {t.live_camera || 'Caméra live'}
                </p>
                <p className="text-[9px] font-bold text-brand-blue-bright uppercase tracking-tighter mt-1">
                  {t.scan_id_desc || 'Scanner en temps réel'}
                </p>
              </div>
            </button>
          ...
          <div className="p-6 flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="relative rounded-lg overflow-hidden border-2 border-brand-green-bright">
              ...
              <div className="absolute top-2 right-2 bg-brand-green-bright text-white px-2 py-1 rounded text-[9px] font-black uppercase tracking-tighter">
                {t.capture_valid || 'Capture validée'}
              </div>
            </div>
            ...
            <div className="bg-brand-green-light/10 dark:bg-brand-green-bright/5 rounded-xl p-4 border border-brand-green-bright/20">
              <p className="text-[9px] font-black text-brand-green-bright uppercase tracking-widest mb-3">
                {t.extraction_done || 'Extraction terminée'}
              </p>
              <div className="space-y-2">
                {Object.entries(ocrData)
                  .filter(([key]) => key !== 'confiance')
                  .map(([key, value]) => (
                    <div
                      key={key}
                      className="flex justify-between items-start gap-4 pb-2 border-b border-brand-green-bright/5 last:border-0 last:pb-0"
                    >
                      <span className="text-[9px] font-bold text-slate-500 uppercase tracking-tighter shrink-0">
                        {FIELD_LABELS[key] || key}
                      </span>
                      <span
                        className={`text-xs font-black text-slate-900 dark:text-white break-all ${
            ...
            <div className="flex gap-3">
              <Btn variant="secondary" icon={RotateCcw} onClick={() => setPhase('choose')} fullWidth>
                {t.restart || 'Recommencer'}
              </Btn>
              <Btn
                variant="success"
                icon={CheckCircle2}
                onClick={() => onDataExtracted(ocrData, capturedImage)}
                fullWidth
              >
                {t.validate_data || 'Valider'}
              </Btn>
            </div>
          </div>
        )}
        ...
        {phase === 'done' && ocrData?.error && (
          <div className="p-6 flex flex-col gap-4 animate-in fade-in duration-500">
            <div className="bg-red-50 dark:bg-red-900/20 rounded-xl p-6 border border-red-200 dark:border-red-800/50 text-center">
              ...
              <Btn variant="secondary" icon={RotateCcw} onClick={() => setPhase('choose')}>
                {t.restart || 'Recommencer'}
              </Btn>
            </div>
          </div>
